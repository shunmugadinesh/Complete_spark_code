 ...
 */   
    ...
    println("Convert the scala method to UDF to use in Dataframe")
...
*/    
 ...
 //https://spark.apache.org/docs/2.3.0/api/sql/
    ...
    //val udfgeneratemailid=udf(reusableobj1.generatemailid _) // if need to use in dsl
      spark.udf.register("udfgeneratemailid",reusableobj1.generatemailid _) // if need to us in sql
...
val dflookupsql = spark.sql("""select a.custid,b.amt,b.transtype,case when b.transtype is null then "old customer" else "repeating customer" end as TransactionorNot
...
val dfjoinsqlkeygen = spark.sql("""select * from (select row_number() over(partition by custid order by dt desc) as transorder,
  ...
  println("Aggregation on the joined data set")    
    val dfjoinsqlagg = spark.sql("""select city,cat,profession,transtype,sum(amt),avg(amt) from joinedview 
...
dfjoinsql.write.option("mode","append").partitionBy("dt").saveAsTable("default.customerhive2")
