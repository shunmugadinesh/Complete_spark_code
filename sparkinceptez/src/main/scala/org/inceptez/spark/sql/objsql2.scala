...
dfcsv.coalesce(1).write.mode("overwrite").option("header","true").option("timestampFormat","yyyy-MM-dd HH:mm:ss").option("nullValue","nullval").option("delimiter","~").
...
option("timestampFormat","yyyy-MM-dd HH:mm:ss").option("dateFormat","yyyy-MM-dd").option("columnNameOfCorruptRecord", "unwantedjsondata").
...
//spark.sql("insert into default.dfdbviewhive select * from dfdbview")
